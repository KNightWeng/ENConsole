...
from oauth import ENConsoleAuth
from storage import Storage
from log import logging

class ENConsole(object):
    ...
    def getStorage(self):
        if ENConsole.storage:
            return ENConsole.storage

        ENConsole.storage = Storage()
        return ENConsole.storage

    def getUserStore(self):
        if ENConsole.userStore:
            return ENConsole.userStore

        userStoreHttpClient = THttpClient.THttpClient(self.userStoreUri)
        userStoreProtocol = TBinaryProtocol.TBinaryProtocol(userStoreHttpClient)
        ENConsole.userStore = UserStore.Client(userStoreProtocol)
    ...
    def getNoteStore(self):
        if ENConsole.noteStore:
            return ENConsole.noteStore

        noteStoreUrl = self.getUserStore().getNoteStoreUrl(self.authToken)
        noteStoreHttpClient = THttpClient.THttpClient(noteStoreUrl)
        noteStoreProtocol = TBinaryProtocol.TBinaryProtocol(noteStoreHttpClient)
        ENConsole.noteStore = NoteStore.Client(noteStoreProtocol)
    ...
    @EdamException
    def createImgNote(self, title, content, resource, tags=None, notebook=None, created=None):
	...
	note.updated = note.created
  	note.resources = [ resource ]
        ...
        logging.debug("New Img note : %s", note)
...
class Notebooks(EverNoteConnector):
    """ Work with auth Notebooks """
...
def main(args=None):
	...
	# Upload image
	if COMMAND == 'img-upload':
	    Notes().imgupload(**ARGS)
